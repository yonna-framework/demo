 ...
 * @return array
 */
function curl_header($data = null)
{
    $clientIP = curl_remoteIP() ?: '127.0.0.1';
    $header = array(
        "CLIENT-IP:{$clientIP}",
 ...
 * @return string
 */
function curl_post($host, $data = array(), $timeout = 10)
{
    if (!$data) $data = array();
    $ch = curl_init();
...
function curl_postSSL($host, $data, $cert, $key, $ca, $timeout = 10)
{
 ...
 * @return mixed
 */
function curl_postStream($host, $data = '', $timeout = 10)
{
    if (!$data) $data = '';
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $host);
