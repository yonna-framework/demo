 ...
 * @package App\Scope
 */
class LeagueAssociateHobby extends AbstractScope
{

    const TABLE = 'league_associate_hobby';
    ...
    public function attach(): array
    {
        ...
        $datas = [];
        foreach ($values as $v) {
            if (!isset($datas[$v['league_associate_hobby_league_id']])) {
                $datas[$v['league_associate_hobby_league_id']] = [];
            }
            $datas[$v['league_associate_hobby_league_id']][] = $v['league_associate_hobby_data_id'];
        }
        unset($values);
        foreach ($tmp as $uk => $u) {
            $tmp[$uk]['league_hobby'] = empty($datas[$u['league_id']]) ? [] : $datas[$u['league_id']];
        }
        if ($isPage) {
            $data['list'] = $tmp;
            return $data;
